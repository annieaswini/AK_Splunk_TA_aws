...
class S3ASLNoticeParser:
    ...
    def parse(self):
        ...
        try:
            ...
            raise TypeError("Unknown AmazonSecurityLake message.") from exc
        return [notification]
        ...
        try:
            ...
            try:
                if self._sqs_sns_validation:
                    logger.debug(f"Validating SNS Signature {message.message_id}")
                    message.is_valid()
                    logger.debug(f"Valid SNS Signature {message.message_id}")
